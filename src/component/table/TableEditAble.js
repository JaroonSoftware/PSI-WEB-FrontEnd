...
export const EditableCell = ({
  ...
  const selectPGforJo = async (data, v) => {
    try {
      setEditing(false);
  ...
  const findHorizontal = (startTd, stepKey) => {
    ...
    while (cur && cur[stepKey]) {
      cur = cur[stepKey];
      if (cur.getAttribute && cur.getAttribute("data-editable") === "1")
        return cur;
    }
    return null;
  };
        ...
        <>
          ...
          {type === "select-pg-jo" && (
            <Select
              ...
              filterOption={filterOption}
              onSelect={(data, value) => selectPGforJo(data, value)}
              onBlur={save}
              disabled={readonly}
            />
          )}
